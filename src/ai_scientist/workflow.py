...
from ai_scientist.agents import AdvisorAgent
from ai_scientist.agents import ReviewerAgent
from ai_scientist.agents import StudentAgent
from ai_scientist.config import AppSettings
from ai_scientist.schemas import AgentMessage
from ai_scientist.schemas import ArtifactRecord
from ai_scientist.schemas import ExperimentGateDecision
from ai_scientist.schemas import ExperimentGateReview
from ai_scientist.schemas import ReviewDecision
from ai_scientist.schemas import WorkflowStatus
from ai_scientist.storage import ArtifactManager
from ai_scientist.storage import MessageBus
from ai_scientist.tools import build_tools
...
class WorkflowState(TypedDict, total=False):
    ...
    waiting_for_agent: str | None
    suspend_reason: str | None
    submitted_artifact_path: str | None
    submitted_artifact_type: str | None
    resume_token: str | None
    last_reply_message_id: str | None
    conversation_round: int
    innovation_review_round: int
    paper_review_round: int
    blind_review_round: int
    literature_notes: list[dict[str, Any]]
    literature_report_path: str | None
    innovation_cards: list[dict[str, Any]]
    approved_innovation: dict[str, Any] | None
    innovation_report_path: str | None
    experiment_plan_path: str | None
    experiment_result_path: str | None
    result_analysis_path: str | None
    manuscript_path: str | None
    advisor_review_path: str | None
    reviewer_review_path: str | None
...
class ResearchWorkflowApp:
    def __init__(self, settings: AppSettings) -> None:
        ...
        self.artifact_manager = ArtifactManager(settings.workspace_root)
        ...
        self.student_agent = StudentAgent(settings=settings, tools=self.tools)
        self.advisor_agent = AdvisorAgent(settings=settings, tools=self.tools)
        self.reviewer_agent = ReviewerAgent(settings=settings, tools=self.tools)
    ...
    def advisor_innovation_review_react_node(self, state: WorkflowState) -> dict[str, Any]:
        ...
        update = {
            ...
            "current_stage": "experiment_design" if approved_innovation else "innovation_revision",
            "active_agent": "student",
            "suspend_reason": None,
        }
        update.update(self._register_artifact(state, artifact))
