...
def build_tools(artifact_manager: ArtifactManager) -> list:  # 根据当前工作区构建一组可供 Agent 调用的工具。
    ...
    def search_workspace(query: str, bucket: str = "workspace") -> str:  # 定义在工作区里全文搜索的工具函数。
        """在工作区文本文件中搜索关键词。"""  # 描述工具用途，帮助模型回查已有文档。
        root = artifact_manager.workspace_root if bucket == "workspace" else artifact_manager.workspace_root / bucket  # 计算需要搜索的根目录。
